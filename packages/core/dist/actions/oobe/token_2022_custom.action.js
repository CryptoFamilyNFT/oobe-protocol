...
const token_2022_custom = {
    ...
    examples: [
        ...
        name: zod_1.z.string().min(4).max(32).describe("Name of the token"),
        symbol: zod_1.z.string().min(2).max(16).describe("Ticker symbol of the token"),
        decimals: zod_1.z.number().min(0).max(18).describe("Number of decimal places"),
        supply: zod_1.z.number().min(1).max(1000000000000).describe("Initial supply of the token"),
        description: zod_1.z.string().min(1).max(250).describe("Description of the token. max 250 characters"),
        feeBasisPoints: zod_1.z.number().min(0).max(10000).describe("Transfer fee in basis points two decimals 700 = 7%"),
        maxFeeInTokens: zod_1.z.number().min(0).max(100000000000).describe("Maximum fee in tokens"),
        pinataKey: zod_1.z.string().describe("Pinata key for uploading metadata"),
        imageUrl: zod_1.z.string().url().describe("Image URL for the token"),
    }),
    handler: async (agent, input) => {
        console.log("[!]: ", input);
        const ob = new oobe_operation_1.OobeOperation(agent);
...
exports.default = token_2022_custom;
//# sourceMappingURL=token_2022_custom.action.js.map