...
import { AIMessage, HumanMessage, SystemMessage, SystemMessageFields } from "@langchain/core/messages";

const m_m = (agentName: string, oobeWan: any, degenVibeX: any) => {
    return `
...
async function createAgentExecution(oobe: OobeCore, agentName: string, memory: MemorySaver) {
    const agent = oobe.getAgent();
    agent.registerActions(Actions.map((action) => action.action)); // Register actions
    ...
    const oobeAgent = createReactAgent({
        llm: agent.genAi(),
...
async function runAgents(oobe1: OobeCore, oobe2: OobeCore) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const field: SystemMessageFields = {
...
Your counterpart is not just a bot — treat them like a mate but if hostile change the way to speak in a podcast or a friend in a Twitter space. Banter. Share alpha. Drop wisdom. Make it memorable.`
        ...
        const stream = await currentAgent.stream(
            { messages: [new SystemMessage(field), new HumanMessage(response)] },
            { configurable: { thread_id: "ONGOING_CONVO" } }
        );
...
function main() {
    ...
    const oobe1 = new OobeCore(config1); // Core for Agent 1
    oobe1.start();

    const oobe2 = new OobeCore(config2); // Core for Agent 2
    oobe2.start();
