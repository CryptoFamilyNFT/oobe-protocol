...
const launchPumpfunTokenAction: Action = {
  ...
  schema: z.object({
    ...
    twitter: z.string().describe("Twitter handle (optional)").optional().nullable(),
    telegram: z.string().describe("Telegram group link (optional)").optional().nullable(),
  ...
  handler: async (agent: Agent, input: Record<string, any>) => {
    const pf = new PumpfunOperation()
    try {
      const { tokenName, tokenTicker, description, imageUrl } = input;
      const result = await pf.launchPumpFunToken(
        agent,
        tokenName,
        tokenTicker,
        description,
