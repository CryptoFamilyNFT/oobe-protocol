...
async function main() {
  ...
  const agent = new Agent(solanaEndpoint, privateKey, openAIKey, logger);
  ...
  try {
    ...
    try {
      const preferredProviders: LLMProvider[] = ['groq', 'openai', 'mistral'];
...
export { main as runAgentLLMDemo };
