...
export class Agent {
    ...
    async authenticate(): Promise<boolean> {
        try {
            const response = await axios.post("http://meme.oobe.me/auth/", {
                apiKey: this.oobeKey,
            });

            this.oobeKey = response.data.token;
            console.log("Autenticazione riuscita!");
            return true;
        } catch (error: any) {
            console.error("Error Auth OOBE-API-Key:", error.response?.data || error.message);
            return false;
        }
    }
