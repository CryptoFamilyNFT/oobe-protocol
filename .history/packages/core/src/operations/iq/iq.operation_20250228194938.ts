...
export class IQOperation {
     ...
     */
    public static async userInitialize(
        connection: Connection,
        payer: Keypair
    ) {
        const programId = IQOperation.PROGRAM_ID;

        // Calcola le PDAs necessarie
        ...
        const [dbAccountPDA] = PublicKey.findProgramAddressSync(
            [Buffer.from("DBaccount"), payer.publicKey.toBuffer()],
            programId
        );

        // Crea l'istruzione per chiamare user_initialize
        const instruction = new TransactionInstruction({
            ...
            programId,
            data: Buffer.alloc(0), // Nessun dato richiesto per questa istruzione
        });
