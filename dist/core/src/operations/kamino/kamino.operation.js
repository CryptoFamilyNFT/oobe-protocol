...
class kaminoOperations {
    constructor(agent) {
        ...
        this.agent = agent;
        this.kamino = this.kaminoInitModule(agent);
    }
    ;
    // Encrypt
    ...
    kaminoInitModule(agent) {
        return new kamino_sdk_1.Kamino('mainnet-beta', agent.connection);
    }
    /**
     *
     */
    ...
    async getAllKaminostrategies() {
        return await this.kamino.getStrategies();
    }
    ...
    async getAssociatedForTokensAndShares(strategy) {
        ...
        const [sharesAta, sharesMintData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, whirlpoolStrategy.sharesMint, this.agent.wallet.publicKey);
        const [tokenAAta, tokenAData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, whirlpoolStrategy.tokenAMint, this.agent.wallet.publicKey);
        const [tokenBAta, tokenBData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, whirlpoolStrategy.tokenBMint, this.agent.wallet.publicKey);
        return {
            sharesAta,
            sharesMintData,
            tokenAAta,
    ...
    async depositShares(strategyPubkey, amountUSDH, amountUSDC) {
        ...
        let tx = (0, kamino_sdk_1.createTransactionWithExtraBudget)(this.agent.wallet.publicKey.toBuffer().length);
        ...
        tx = await (0, kamino_sdk_1.assignBlockInfoToTransaction)(this.agent.connection, tx, this.agent.wallet.publicKey);
    ...
    async withdrawShares(strategyPubkey, amountUSDH, amountUSDC) {
        ...
        const [sharesAta, sharesMintData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, strategy.sharesMint, this.agent.wallet.publicKey);
        const [tokenAAta, tokenAData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, strategy.tokenAMint, this.agent.wallet.publicKey);
        const [tokenBAta, tokenBData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, strategy.tokenBMint, this.agent.wallet.publicKey);
    ...
    async CREATE_CAMINO_STRATEGY(PoolAddress, isOrca) {
        ...
        let tx = (0, kamino_sdk_1.createTransactionWithExtraBudget)(signer.publicKey.toBuffer().length);
        ...
        switch (whirlpoolHandle) {
            ...
            case "Raydium":
                ...
                tokenMintA = raydiumPoolState.tokenMintA;
                tokenMintB = raydiumPoolState.tokenMintB;
                break;
            default:
        ...
        const [tokenAAta, tokenAData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, tokenMintA, owner);
        const [tokenBAta, tokenBData] = await (0, kamino_sdk_1.getAssociatedTokenAddressAndData)(this.agent.connection, tokenMintB, owner);
        if (!tokenAData) {
            tx.add((0, kamino_sdk_1.createAssociatedTokenAccountInstruction)(owner, tokenAAta, owner, tokenMintA));
        }
        if (!tokenBData) {
            tx.add((0, kamino_sdk_1.createAssociatedTokenAccountInstruction)(owner, tokenBAta, owner, tokenMintB));
        }
        ...
        const createStrategyIx = await kamino.createStrategy(newStrategy.publicKey, poolState.whirlpoolsConfig, this.agent.wallet.publicKey, isOrca ? "ORCA" : "RAYDIUM");
        tx.add(createStrategyIx);
        tx = await (0, kamino_sdk_1.assignBlockInfoToTransaction)(this.agent.connection, tx, owner);
