...
exports.RaydiumAmmInstructionCoder = exports.RayIxNameForAgents = void 0;
// @ts-nocheck
const B = __importStar(require("@native-to-anchor/buffer-layout"));
exports.RayIxNameForAgents = [
    "initialize",
    "initialize2",
    "monitorStep",
    "deposit",
...
function encodeInitialize({ nonce, openTime, }) { return encodeData({ initialize: { nonce, openTime, } }, 1 + 1 + 8); }
function encodeInitialize2({ nonce, openTime, initPcAmount, initCoinAmount, }) { return encodeData({ initialize2: { nonce, openTime, initPcAmount, initCoinAmount, } }, 1 + 1 + 8 + 8 + 8); }
function encodeMonitorStep({ planOrderLimit, placeOrderLimit, cancelOrderLimit, }) { return encodeData({ monitorStep: { planOrderLimit, placeOrderLimit, cancelOrderLimit, } }, 1 + 2 + 2 + 2); }
function encodeDeposit({ maxCoinAmount, maxPcAmount, baseSide, }) { return encodeData({ deposit: { maxCoinAmount, maxPcAmount, baseSide, } }, 1 + 8 + 8 + 8); }
function encodeWithdraw({ amount, }) { return encodeData({ withdraw: { amount, } }, 1 + 8); }
function encodeMigrateToOpenBook({}) { return encodeData({ migrateToOpenBook: {} }, 1); }
function encodeSetParams({ param, value, newPubkey, fees, lastOrderDistance, needTakeAmounts, }) { return encodeData({ setParams: { param, value, newPubkey, fees, lastOrderDistance, needTakeAmounts, } }, 1 + 1 + 1 + (value === null ? 0 : 8) + 1 + (newPubkey === null ? 0 : 32) + 1 + (fees === null ? 0 : 64) + 1 + (lastOrderDistance === null ? 0 : 16) + 1 + (needTakeAmounts === null ? 0 : 16)); }
function encodeWithdrawPnl({}) { return encodeData({ withdrawPnl: {} }, 1); }
function encodeWithdrawSrm({ amount, }) { return encodeData({ withdrawSrm: { amount, } }, 1 + 8); }
function encodeSwapBaseIn({ amountIn, minimumAmountOut, }) { return encodeData({ swapBaseIn: { amountIn, minimumAmountOut, } }, 1 + 8 + 8); }
function encodePreInitialize({ nonce, }) { return encodeData({ preInitialize: { nonce, } }, 1 + 1); }
function encodeSwapBaseOut({ maxAmountIn, amountOut, }) { return encodeData({ swapBaseOut: { maxAmountIn, amountOut, } }, 1 + 8 + 8); }
function encodeSimulateInfo({ param, swapBaseInValue, swapBaseOutValue, }) { return encodeData({ simulateInfo: { param, swapBaseInValue, swapBaseOutValue, } }, 1 + 1 + 1 + (swapBaseInValue === null ? 0 : 16) + 1 + (swapBaseOutValue === null ? 0 : 16)); }
function encodeAdminCancelOrders({ limit, }) { return encodeData({ adminCancelOrders: { limit, } }, 1 + 2); }
function encodeCreateConfigAccount({}) { return encodeData({ createConfigAccount: {} }, 1); }
function encodeUpdateConfigAccount({ param, owner, }) { return encodeData({ updateConfigAccount: { param, owner, } }, 1 + 1 + 32); }
